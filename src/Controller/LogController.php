...
class LogController extends AbstractController {
	...
	public function newLogAjax(Request $request) {
		...
		//$status = 99;
		//return new JsonResponse(array('status' => $status, 'message' => $message . "<br> idf =" . $idf));
		$station = $user->getStationFromId($ids);
		$folder = $station->getFolderFromId( $idf );
		...
		//if ($form->isSubmitted() && $form->isValid()) {
			...
			//$template = $this->get('twig')->render('/templates/log/row.html.twig', [ 'log' => $data ] );
		...
		$template = $this->renderView('log/row.html.twig', [ 'log' => $data ] );
		//$template = "<tr><td colspan=\"13\">AHOJ</td></tr>";
	...
	public function deleteAjaxAction(Request $request) {
		...
		if ($request->isXMLHttpRequest()) {
			//return new $this->renderView('homepage/default.html.twig');
			return new JsonResponse(array('status' => $status, 'data' => $id, 'message' => $message));
		}
	}
